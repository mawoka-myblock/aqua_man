...
{:then food_available}
    ...
    {:else}
        ...
        <p class="text-center w-screen text-sm italic mb-2 -mt-1">Du musst erst einen Futter-Typen haben, bevor du
            Futter hinzufügen kannst!</p>
    {/if}
{/await}
{#await foods_promise}
    <Spinner/>
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                {#each foods as food}
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 cursor-pointer"
                        on:click={() => {window.location.assign(`/food/view/${food.id}`)}}>
                        <th scope="row"
