...
{#if feedingModalOpen}
	...
	{:then _}
		<div class="fixed top-0 left-0 z-50 w-full h-full visible bg-white">
			<button
				class="bg-gray-200 rounded-lg p-1 m-2 hover:bg-gray-300 transition group"
				on:click={() => {
					feedingModalOpen = false;
				}}
			>
				<svg
					class="w-8 h-8 text-gray-400 group-hover:text-gray-500 transition"
					fill="none"
					stroke="currentColor"
					viewBox="0 0 24 24"
			...
			<div class="flex justify-center w-full h-full">
				<form
					class="my-auto bg-gray-300 py-4 px-6 flex flex-col"
					on:submit|preventDefault={save_feeding}
				>
							...
							{#each search_results as res}
								<li>
									<button
										class="w-full"
										on:click={() => {
											food_query = `${res.name} von ${res.hersteller}`;
											selected_food_id = res.id;
											search_results = [];
											search_suggestions = [];
										}}
									>
					...
					<button
						type="submit"
						class="py-1 px-3 bg-green-300 rounded-lg disabled:opacity-50 mt-4"
						disabled={!selected_food_id}
					>
						Speichern!
					</button>
				</form>
